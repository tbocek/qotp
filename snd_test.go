...
func TestSndReadyToRetransmit(t *testing.T) {
	...
	data, offset, err := sb.ReadyToRetransmit(1, nil, 1000, 50, Data, 200) // RTO = 50, now = 200. 200-100 > 50
	assert.Nil(t, err)
	assert.Equal(t, []byte("test1"), data)
	assert.Equal(t, uint64(0), offset)

	data, offset, err = sb.ReadyToRetransmit(2, nil, 1000, 100, Data, 200) // RTO = 100, now = 200. 200-100 = 100, thus not ready yet
	assert.Nil(t, err)
	assert.Nil(t, data)
	assert.Equal(t, uint64(0), offset)

	data, offset, err = sb.ReadyToRetransmit(1, nil, 1000, 99, Data, 399) // RTO = 99, now = 399. 399-100 > 99 with backoff
	assert.Nil(t, err)
	assert.Equal(t, []byte("test1"), data)
	assert.Equal(t, uint64(0), offset)
	...
	data, offset, err = sb2.ReadyToRetransmit(1, nil, 20, 99, Data, 200) // Small MTU should trigger split
	assert.Nil(t, err)
...
func TestSndMultipleStreams(t *testing.T) {
	...
	data2, offset2 := sb.ReadyToSend(2, Data, nil, 1000, 00)
...
func TestSndAcknowledgeGaps(t *testing.T) {
	...
	sb.ReadyToSend(1, Data, nil, 43, 100) // offset 0, len 4
	sb.ReadyToSend(1, Data, nil, 43, 100) // offset 4, len 4
	sb.ReadyToSend(1, Data, nil, 43, 100) // offset 8, len 4
...
func TestSndRetransmitWithGaps(t *testing.T) {
	...
	sb.ReadyToSend(1, Data, nil, 44, 100) // sends "01234"
	sb.ReadyToSend(1, Data, nil, 44, 100) // sends "56789"
	...
	data, offset, err := sb.ReadyToRetransmit(1, nil, 44, 50, Data, 200)
	assert.Nil(t, err)
