...
func (c *Connection) rtoNano() uint64 {
	...
	//the first packet
	if rto == 0 {
		return 200 * msNano //with backoff, max 6.2 sec
	} else if rto < 100 * msNano {
		return 100 * msNano //do not go below 100ms
	} else if rto > 2000 * msNano {
		return 2000 * msNano //everything is reachable via 2sec
	}

	return rto
}

func (c *Connection) UpdateRTT(rttMeasurementNano uint64) {
	c.mu.Lock()
	defer c.mu.Unlock()
